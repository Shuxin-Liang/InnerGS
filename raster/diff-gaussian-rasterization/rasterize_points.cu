	...
	const int axis)
{
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward(
		...
		debug,
		axis);  // Use passed axis parameter
  }
	...
	const int axis)
{
  ...
  if(dL_dout_depth.size(0) != 0)
  {
	...
	dL_dout_invdepthptr = dL_dout_depth.data<float>();
  }
  ...
  {  
	  ...
	  if(sh.defined() && sh.size(0) > 0)
	  {
		  sh_ptr = sh.contiguous().data<float>();
	  }
  ...
  // Create empty dL_dmeans2D tensor to maintain return value compatibility
